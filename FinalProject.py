...
with st.sidebar:
    ...
    selected = option_menu('Disease Diagnosis 3.0',
    ...
    if st.button('Diabetes Test Result'):
        try: 
            ...
            if not Pregnancies or not Glucose or not BloodPressure or not SkinThickness or not Insulin or not BMI or not DiabetesPedigreeFunction or not Age:
                st.error("Enter all the field value")
            else:
                st.error("Value should be number")
...
if (selected == 'Heart Disease Prediction'):
    ...
    if st.button('Heart Disease Test Result'):
        try: 
            data_format = np.array([[int(age), int(sex), int(cp), int(trestbps), int(chol), int(fbs), int(restecg),int(thalach),int(exang),float(oldpeak),int(slope),int(ca),int(thal)]])
            heart_prediction = heart_disease_model.predict(data_format)
            ...
            if not age or not sex or not cp or not trestbps or not chol or not fbs or not restecg or not thalach or not exang or not oldpeak or not slope or not ca or not thal:
                ...
                st.error("Value should be a number")
